...
import requests
import unidecode
from Maps_cultural_life_Paris.Cinema.UnitScrap import UnitScrapCinema
...
def ScrapCinema(url,day,HEADERS = HeadersDefault):
    ...
    dfList = []                                           #Adding each Dataframe from pages
    ...
    ref = pd.read_csv('Maps_cultural_life_Paris/Cinema/ListeCinema.csv', sep = ';')          #Merging with an opendata list to get adress
    ...
    merged_df.to_csv('Maps_cultural_life_Paris/Outputs/DataSets/DataCinema.csv', index=False)
    return merged_df

if __name__ == '__main__':
    url = 'https://www.cinefil.com/seances-cinema/paris'
    ScrapCinema(url,'27/12')